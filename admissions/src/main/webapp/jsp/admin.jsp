...
<body>
	...
	<c:forEach items="${registeredEntrants}" var="currentRegistration">
		<c:choose>
			...
			<c:otherwise>
				<c:set var="hasDenied" value="${true}" />
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<div class="container">
		...
		<c:choose>
			<c:when test="${hasAccepted == true}">

				<h2>
					<spring:message code="admin.accepted_title" />
				</h2>
								...
								<c:when test="${currentRegistration.isEnrolled}">
									<tr class="hide-show" style="cursor: pointer">
										...
										<td align="center"><c:choose>
												<c:when
													test="${currentRegistration.user.imageBase64 != null}">
													<img src="data:image/png;base64, ${currentRegistration.user.imageBase64}"
														width="90" height="60">
												</c:when>
												<c:otherwise>
													<img
										...
										<td align="center"><spring:message code="admin.yes"/></td>
									</tr>
		...
		<c:choose>
			<c:when test="${hasDenied == true}">
				<h2>
					<spring:message code="admin.denied" />
				</h2>
				<table class="table table-bordered">
					<thead>
							...
							<c:choose>
								<c:when test="${!currentRegistration.isEnrolled}">
									<tr class="hide-show" style="cursor: pointer">
										...
										<td align="center"><spring:message code="admin.no"/></td>
									</tr>
